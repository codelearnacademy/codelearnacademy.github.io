...
route: maven
lesson_id: leccion26
lesson_file: 26-dependencias-vs-plugins
lesson_number: "26"
title: Dependencias frente a plugins
description: Diferencia el código que usa tu aplicación de las herramientas que usa Maven para construirla.
permalink: /maven/leccion26/
lessons:
...
---
# Dependencias frente a plugins

Diferencia el código que usa tu aplicación de las herramientas que usa Maven para construirla.

## Qué vas a conseguir

- Comprender **dependency** en el contexto de un proyecto Maven.
- Comprender **plugin** en el contexto de un proyecto Maven.
- Comprender **build** en el contexto de un proyecto Maven.
- Comprender **goals** en el contexto de un proyecto Maven.
...
Maven se entiende mejor cuando cada concepto se conecta con una necesidad concreta del proyecto. En esta lección nos centraremos en **dependency** y observaremos su efecto en un proyecto Java sencillo.
...
```text
Dependencia → la usa tu aplicación
Plugin      → lo usa Maven durante el build
```
...
En Gradle también distinguimos dependencias del proyecto y plugins del build. En Node, algunas herramientas de build se instalan como dependencias de desarrollo, por lo que la separación conceptual existe aunque la representación sea distinta.
...
1. Clasifica JUnit, Gson y Maven Compiler Plugin.
2. Explica por qué un plugin no se importa desde tu código Java.
...
<div class="cla-lesson-nav">
  <a href="/maven/leccion25/">← 25 · El build como barrera de calidad</a>
