...
route: maven
lesson_id: leccion36
lesson_file: 36-que-versionar-git
lesson_number: "36"
title: Qué versionar con Git
description: Distingue archivos fuente, configuración y artefactos generados en un proyecto Maven.
...
Distingue archivos fuente, configuración y artefactos generados en un proyecto Maven.

## Qué vas a conseguir

- Comprender **Git** en el contexto de un proyecto Maven.
- Comprender **versionado** en el contexto de un proyecto Maven.
- Comprender **fuente vs generado** en el contexto de un proyecto Maven.
...
Maven se entiende mejor cuando cada concepto se conecta con una necesidad concreta del proyecto. En esta lección nos centraremos en **Git** y observaremos su efecto en un proyecto Java sencillo.
...
```text
Versionar: pom.xml, src/, mvnw, .mvn/
No versionar: target/
```
...
La misma regla aparece en Node (`node_modules/`, `dist/`), Python (`__pycache__/`, entornos virtuales) y Go (binarios generados).
...
1. Clasifica diez archivos de un proyecto Maven.
2. Explica por qué `target/` no debe almacenarse normalmente en Git.
