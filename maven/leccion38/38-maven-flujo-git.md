...
route: maven
lesson_id: leccion38
lesson_file: 38-maven-flujo-git
lesson_number: "38"
title: Maven dentro del flujo Git
description: Integra build, pruebas y commits en un flujo de trabajo disciplinado.
permalink: /maven/leccion38/
lessons:
...
Integra build, pruebas y commits en un flujo de trabajo disciplinado.

## Qué vas a conseguir

- Comprender **branch** en el contexto de un proyecto Maven.
- Comprender **test before commit** en el contexto de un proyecto Maven.
- Comprender **verify** en el contexto de un proyecto Maven.
...
Maven se entiende mejor cuando cada concepto se conecta con una necesidad concreta del proyecto. En esta lección nos centraremos en **branch** y observaremos su efecto en un proyecto Java sencillo.
...
```text
rama → cambios → ./mvnw test → commit → push
```
...
En cualquier stack profesional conviene ejecutar pruebas antes de publicar cambios. El comando cambia (`npm test`, `pytest`, `go test`) pero la práctica es la misma.
...
1. Crea una rama, implementa un cambio y verifica antes del commit.
2. Provoca un test fallido y explica por qué no deberías publicar el cambio.
...
<div class="cla-lesson-nav">
  <a href="/maven/leccion37/">← 37 · .gitignore para Maven</a>
