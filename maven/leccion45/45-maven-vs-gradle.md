...
route: maven
lesson_id: leccion45
lesson_file: 45-maven-vs-gradle
lesson_number: "45"
title: Maven frente a Gradle
description: Compara dos herramientas habituales de construcción dentro del ecosistema JVM.
permalink: /maven/leccion45/
lessons:
...
Compara dos herramientas habituales de construcción dentro del ecosistema JVM.

## Qué vas a conseguir

- Comprender **convención** en el contexto de un proyecto Maven.
- Comprender **POM** en el contexto de un proyecto Maven.
- Comprender **build.gradle** en el contexto de un proyecto Maven.
- Comprender **tasks** en el contexto de un proyecto Maven.
...
Maven se entiende mejor cuando cada concepto se conecta con una necesidad concreta del proyecto. En esta lección nos centraremos en **convención** y observaremos su efecto en un proyecto Java sencillo.
...
```text
Maven → lifecycle + XML + convención fuerte
Gradle → tareas + DSL + mayor flexibilidad
```
...
Ambos resuelven dependencias y builds en Java/JVM, pero con modelos distintos. Comprender Maven facilita posteriormente aprender Gradle.
...
1. Compara un `pom.xml` y un `build.gradle` básicos.
2. Explica una ventaja de convención y otra de flexibilidad.
