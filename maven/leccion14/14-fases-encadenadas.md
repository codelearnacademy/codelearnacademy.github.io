...
route: maven
lesson_id: leccion14
lesson_file: 14-fases-encadenadas
lesson_number: "14"
title: Fases encadenadas
description: Aprende por qué ejecutar una fase implica ejecutar también las anteriores.
permalink: /maven/leccion14/
lessons:
...
Aprende por qué ejecutar una fase implica ejecutar también las anteriores.

## Qué vas a conseguir

- Comprender **orden del lifecycle** en el contexto de un proyecto Maven.
- Comprender **fases previas** en el contexto de un proyecto Maven.
- Comprender **build reproducible** en el contexto de un proyecto Maven.
...
Maven se entiende mejor cuando cada concepto se conecta con una necesidad concreta del proyecto. En esta lección nos centraremos en **orden del lifecycle** y observaremos su efecto en un proyecto Java sencillo.
...
```bash
mvn package
# ejecuta las fases anteriores necesarias
```
...
Este comportamiento contrasta con herramientas basadas en scripts independientes, donde el desarrollador debe encadenar explícitamente las tareas.
...
1. Predice qué ocurrirá al ejecutar `mvn verify`.
2. Explica por qué `mvn package` puede fallar por una prueba.
