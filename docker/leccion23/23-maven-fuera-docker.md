...
route: docker
lesson_id: leccion23
lesson_file: 23-maven-fuera-docker
lesson_number: "23"
title: Maven fuera de Docker
description: Evalúa el patrón en el que el host construye el JAR y Docker solo empaqueta.
permalink: /docker/leccion23/
lessons:
...
---
# Maven fuera de Docker

Evalúa el patrón en el que el host construye el JAR y Docker solo empaqueta.

## Qué vas a conseguir

- Comprender **build externo**.
- Comprender **artefacto**.
- Comprender **pipeline**.
...
Es sencillo y muy frecuente en CI.
...
En Angular puede hacerse lo mismo: `npm run build` fuera de Docker y después copiar `dist/` a Nginx.
...
1. Describe ventajas e inconvenientes del patrón.
2. Implementa el build en dos pasos y comprueba que la imagen solo recibe el artefacto.
