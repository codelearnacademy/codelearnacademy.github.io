...
route: docker
lesson_id: leccion22
lesson_file: 22-contenedorizacion-jar
lesson_number: "22"
title: Contenerizar un JAR
description: Empaqueta una aplicación Java ya construida dentro de una imagen.
permalink: /docker/leccion22/
lessons:
...
Empaqueta una aplicación Java ya construida dentro de una imagen.

## Qué vas a conseguir

- Comprender **Maven package**.
- Comprender **JAR**.
...
```bash
./mvnw clean package
docker build -t codelearn/api:1.0.0 .
docker run --rm -p 8080:8080 codelearn/api:1.0.0
```
...
Maven produce el artefacto; Docker produce el entorno desplegable. Cada herramienta mantiene una responsabilidad clara.
...
1. Conteneriza una API o aplicación Java sencilla.
2. Comprueba que se ejecuta sin usar el IDE.
