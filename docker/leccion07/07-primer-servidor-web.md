...
route: docker
lesson_id: leccion07
lesson_file: 07-primer-servidor-web
lesson_number: "07"
title: Primer servidor web
description: Publica un servicio HTTP dentro de un contenedor y accede desde el host.
permalink: /docker/leccion07/
lessons:
...
Publica un servicio HTTP dentro de un contenedor y accede desde el host.

## Qué vas a conseguir

- Comprender **nginx**.
- Comprender **puertos**.
- Comprender **docker run -p**.
- Comprender **servicio HTTP**.
...
Abre `http://localhost:8080` y relaciona puerto del host con puerto del contenedor.
...
Más adelante usaremos Nginx también para servir el resultado compilado de Angular y, opcionalmente, actuar como reverse proxy hacia Spring.
...
1. Arranca el mismo servicio en el puerto 8081.
2. Intenta reutilizar 8080 con otro contenedor y explica el error.
