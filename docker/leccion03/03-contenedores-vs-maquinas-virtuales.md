...
route: docker
lesson_id: leccion03
lesson_file: 03-contenedores-vs-maquinas-virtuales
lesson_number: "03"
title: Contenedores frente a máquinas virtuales
description: Diferencia contenedores y máquinas virtuales sin reducir Docker a una simple VM ligera.
permalink: /docker/leccion03/
lessons:
...
Diferencia contenedores y máquinas virtuales sin reducir Docker a una simple VM ligera.

## Qué vas a conseguir

- Comprender **kernel compartido**.
- Comprender **hipervisor**.
- Comprender **sistema invitado**.
- Comprender **aislamiento**.
...
```text
VM: aplicación → SO invitado → hipervisor → host
Contenedor: aplicación → runtime de contenedores → kernel del host
```
...
Vagrant y máquinas virtuales resuelven reproducibilidad de forma distinta. Docker busca empaquetar procesos y dependencias con menor coste que una VM completa.
...
1. Dibuja ambas arquitecturas.
2. Explica por qué una VM puede ejecutar un kernel diferente y un contenedor normalmente no.
